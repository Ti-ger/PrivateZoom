...
def get_abstracted_data():
    ...
    if config.get("ENFORCE_PRIVACY", False):
        ...
        if config.get("DELETE_TRACES", False):
            ...
            df = delete_trace(df,f"{FILEPATH}/max_zoom.xes", config.get("K_TRACE", -1), config.get("K_EVENT", -1), config.get("K_EDGE", -1), config.get("L_DIV", 1), config.get("SINGLE_EVENT_L_DIV", False), config.get("FOlLOW_EVENT_L_DIV", False))
        else:
